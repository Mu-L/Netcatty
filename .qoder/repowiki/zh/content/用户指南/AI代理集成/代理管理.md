...
AH["acpHistory.ts"]
SIAT["SettingsAITab.tsx"]
CBC["CodebuddyCard.tsx"]
end
subgraph "基础设施"
MA["managedAgents.ts"]
T["types.ts"]
...
subgraph "主进程桥接"
AB["aiBridge.cjs"]
ADH["agentDiscoveryHandlers.cjs"]
SU["shellUtils.cjs"]
end
AS --> SSD
AS --> MAS
...
participant Resolver as "aiResolveCli"
participant PathUtils as "shellUtils"
participant Proc as "外部代理进程"
UI->>Hook : 触发"重新发现/启用"
Hook->>Bridge : 调用 aiDiscoverAgents()
Bridge-->>Hook : 返回 DiscoveredAgent[]
Hook->>Resolver : 调用 aiResolveCli({command, customPath})
Resolver->>PathUtils : resolveCliFromPath()
PathUtils-->>Resolver : 返回标准化路径
Resolver->>Resolver : 版本检测与可用性验证
Resolver-->>Hook : 返回 AgentPathInfo
...
Hook->>Resolver : aiResolveCli({command, customPath})
Resolver-->>Hook : AgentPathInfo
...
```mermaid
flowchart TD
A["aiResolveCli 调用"] --> B{"是否提供 customPath?"}
B --> |是| C["normalizeCliPathForPlatform(customPath)"]
C --> D{"路径有效?"}
D --> |是| E["使用自定义路径"]
D --> |否| F["resolveCliFromPath(command)"]
B --> |否| G["resolveCliFromPath(command)"]
E --> H["probeCliVersion"]
F --> H
G --> H
H --> I{"版本检测通过?"}
I --> |是| J["返回 AgentPathInfo"]
I --> |否| K["返回不可用状态"]
```
...
Bridge --> ADH["agentDiscoveryHandlers.cjs"]
ADH --> SU["shellUtils.cjs"]
SIAT["SettingsAITab.tsx"] --> MAS
SIAT --> ADH
```
